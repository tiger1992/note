...
public class IdcardUtil {
	...
	public static String getBirthdaybyIdcard(String idcard) {
		...
		// 18位身份证
		if (idcard.length() == 18) {
			tempIdcard = idcard.substring(6, 10) + "-" + idcard.substring(10, 12) + "-" + idcard.substring(12, 14);
		} else {
			tempIdcard = "19"+idcard.substring(6, 8) + "-" + idcard.substring(8, 10) + "-" + idcard.substring(10, 12);
		}
		return tempIdcard;
	}
	...
	private static String convertIdcarBy15bit(String idcard) {
		...
		if (isDigital(idcard)) {
			...
			try {
				...
				birthdate = DateUtils.parseDate(birthday, "yyMMdd");
			} catch (ParseException e) {
				// e.printStackTrace();
				logger.error("时间【{}】解析出错", birthday);
				return null;
			}
